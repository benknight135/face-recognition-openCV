...
while True:
    ...
    rgb = cv2.cvtColor(frame_detect, cv2.COLOR_BGR2RGB)
    rgb = imutils.resize(frame_detect, width=750)
    r = frame_detect.shape[1] / float(rgb.shape[1])
    ...
    for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            cv2.rectangle(frame_detect, (left, top), (right, bottom),
                    (0, 255, 0), 2)
    ...
    if writer is None and args["output"] is not None:
            ...
            writer = cv2.VideoWriter(args["output"], fourcc, 24,
                    (frame_detect.shape[1], frame_detect.shape[0]), True)

    # if the writer is not None, write the frame with recognized
    # faces t odisk
    if writer is not None:
        writer.write(frame_detect)
